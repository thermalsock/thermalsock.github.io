// Node has no localStorage; stub it so the module under test runs unchanged.
let store = {};
...
// half-right run must not lower the best
e = P.recordRun('BoC', 'Fifths 1', ['hit', 'missed', 'hit', 'missed']);
check('best is retained', e.bestAccuracy, 1);
check('last reflects the poor run', e.lastAccuracy, 0.5);
check('run count increments', e.runs, 2);

// pending chords are excluded, not counted as misses
...
// dropped counts against you
e = P.recordRun('BoC', 'Holds', ['hit', 'dropped', 'hit', 'hit']);
check('dropped counts as not-hit', e.lastAccuracy, 0.75);
check('75% is below the 80% pass bar', P.isLessonPassed('BoC', 'Holds'), false);

// a run with nothing scored is ignored entirely
...
check('last played tracked', P.getLastPlayed(), { pack: 'BoC', lesson: 'Long' });

// persistence across a fresh module state
check('export shape', P.exportProgress().kind, 'transient-lab-progress');
P.resetProgress();
check('reset clears everything', P.getSummary(), { attempted: 0, passed: 0, runs: 0 });

...
function generateQuestion(config) {
  if (config.type === "higher_lower") {
    ...
    if (Math.random() < .1) {
      return {
        ...
        prompt: "Is the second tone higher, lower, or the same?"
      };
    }
    ...
    return {
      ...
      a: base,
      b: second,
      correct: ascending ? "higher" : "lower",
      choices: [ "higher", "lower", "same" ],
      prompt: "Is the second tone higher, lower, or the same?"
    };
  }
  if (config.type === "identify_interval") {
    ...
    const choiceSemitones = config.intervals.slice();
    return {
      ...
      correct: iv.name,
      choices: choices,
      choiceSemitones: choiceSemitones,
  ...
  if (config.type === "identify_degree") {
    ...
    const choiceOffsets = config.degrees.slice();
    return {
      type: "identify_degree",
      offset: offset,
      correct: correct,
      choices: choices,
      choiceOffsets: choiceOffsets,
      prompt: "Which scale degree? (First note is the tonic.)",

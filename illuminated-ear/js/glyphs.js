...
export function drawGlyph(ctx, x, y, pitchClass, strength, inkColor, jitterSeed, sizeMult = 1, skew = 0, jitterAmount = 1, variantOverride = null) {
  ...
  const variant = variantOverride != null
    ? (((variantOverride % VARIANTS_PER_CLASS) + VARIANTS_PER_CLASS) % VARIANTS_PER_CLASS)
    : (jitterSeed % VARIANTS_PER_CLASS);
  ...
  const rotJitter = (jitter() - 0.5) * 0.22 * jitterAmount;
  ...
  template.forEach((s) => {
    const dx = (jitter() - 0.5) * 1.2 * jitterAmount;
    const dy = (jitter() - 0.5) * 1.2 * jitterAmount;
...
/**
 * A tiny sparkle mark — four short radiating strokes from a point, plus a
 * small center dot. Deliberately a different mark family from the
 * manuscript glyphs (not just a shrunk-down glyph), used for high-band-
 * dominant material, where a full glyph would read as too heavy for what's
 * actually a bright, thin transient (cymbals, breath noise, fret squeak,
 * a hi-hat, etc.) rather than a note.
 */
export function drawSparkle(ctx, x, y, size, inkColor, alpha, jitterSeed) {
  ...
  for (let i = 0; i < 4; i++) {
    ...
    const len = size * (0.7 + jitter() * 0.6);
    ctx.beginPath();
    ctx.moveTo(Math.cos(a) * len * 0.22, Math.sin(a) * len * 0.22);
...
export function drawBlot(ctx, x, y, strength, inkColor, jitterSeed, jitterAmount = 1) {
  ...
  for (let i = 0; i < dropletCount; i++) {
    ...
    const d = baseR * (0.8 + jitter() * 1.6 * jitterAmount);

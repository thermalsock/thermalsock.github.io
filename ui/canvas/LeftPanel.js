...
export function drawLeftPanel(ctx, canvas) {
    ...
    if (!connected) {
        ...
        if (deviceNames.length === 2)
            ...
            line2 = `+${deviceNames.length - 1} more`;
...
export function hitTestCategorySelector(x, y) {
    const layout = getLeftPanelLayout();
    if (pointInBox(x, y, layout.scalesHalf))
        return "scales";
    if (pointInBox(x, y, layout.genresHalf))
        return "genres";
    return null;
}
export function hitTestScaleTypeSelector(x, y) {
    const layout = getLeftPanelLayout();
    if (!layout.scaleTypeBox)
        return null;
    const stBox = layout.scaleTypeBox;
    if (pointInBox(x, y, stBox))
        return "toggle";
    if (contentState.scaleTypeDropdownOpen) {
        const listStartY = stBox.y + stBox.h + 6;
        for (let i = 0; i < SCALE_TYPES.length; i++) {
            const itemBox = { x: stBox.x, y: listStartY + i * 26, w: stBox.w, h: 24 };
            if (pointInBox(x, y, itemBox))
                return i;
        }
    }
    return null;

...
export function getFader(id) {
  return faderRegistry.get(id) || null;
}

export function getFaderValue(id) {
  const f = faderRegistry.get(id);
  return f ? f.value : undefined;
}

export function setFaderValue(id, v) {
  const f = faderRegistry.get(id);
  if (f) f.setValue(v, false);
}
...
export function getHFaderValue(id) {
  const f = hFaderRegistry.get(id);
  return f ? f.value : undefined;
}

export function setHFaderValue(id, v) {
  const f = hFaderRegistry.get(id);
  if (f) f.setValue(v, false);
}

export function setHFaderLabel(id, text) {
  const f = hFaderRegistry.get(id);
  if (f && f.labelEl) f.labelEl.textContent = text;
}
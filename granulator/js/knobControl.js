// knobControl.js
// A real rotary knob widget: SVG arc track + value arc + pointer, vertical
// drag to turn (matches how every software knob from Ableton to Serum
// behaves), scroll-wheel nudge, double-click to reset, click the numeric
// readout to type an exact value. No dependencies.
...
const knobRegistry = new Map(); // id -> Knob instance, so main.js can look values up uniformly

export class Knob {
  /**
   * @param {HTMLElement} mount - empty container element to build the knob into
   * @param {object} opts
   *   id, label, min, max, step, value, defaultValue, unit,
   *   format(value) -> string, onChange(value), size (px)
   */
  ...
  _build(mount, label, unit) {
    ...
    face.setAttribute('cx', cx); face.setAttribute('cy', cy); face.setAttribute('r', r - 7);
  ...
  _renderAngle() {
    ...
    const inner = polar(this._cx, this._cy, (this._r - 10) * 0.35, angle);
    this.pointer.setAttribute('x1', inner.x); this.pointer.setAttribute('y1', inner.y);
    this.pointer.setAttribute('x2', p.x); this.pointer.setAttribute('y2', p.y);
  }
...
export function getKnob(id) { return knobRegistry.get(id) || null; }
export function getKnobValue(id) { const k = knobRegistry.get(id); return k ? k.value : undefined; }
export function setKnobValue(id, v) { const k = knobRegistry.get(id); if (k) k.setValue(v, false); }

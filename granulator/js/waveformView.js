export class WaveformView {
  constructor(canvas, {onScan: onScan} = {}) {
    this.canvas = canvas;
  ...
  setReadHead(norm) {
    this.readHeadNorm = norm;
  }
  setScanSpread(v) {
    this.scanSpread = v;
  }
  setScanRange(start, end) {
    this.scanRangeStart = start;
    this.scanRangeEnd = end;
  }
  ...
  draw() {
    ...
    if (n > 1) {
      ...
      for (let i = 0; i < n; i++) {
        ...
        const y1 = h / 2 - max * (h / 2) * .92;
        const y2 = h / 2 - min * (h / 2) * .92;
        ctx.moveTo(x, y1);
        ctx.lineTo(x, y2);
      }
      ctx.stroke();
    ...
    if (rangeStart > .001 || rangeEnd < .999) {

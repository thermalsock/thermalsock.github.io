export class Renderer {
  ...
  _strokeTrace(ctx, points, colorOrStops, glow) {
    ...
    const strokeStyle = isGradient
      ? this._makeGradient(ctx, colorOrStops)
      : colorOrStops;
    const glowColor = isGradient
      ? colorOrStops[Math.floor(colorOrStops.length / 2)]
      : colorOrStops;
  ...
  drawSpectrum(
    freqData,
    { sampleRate, fftSize, minDb = -100, maxDb = -30, maxFreqHz = 2e4 } = {},
  ) {
    const { persistCtx, width, height, theme } = this;
    this._fadePersistence();
  ...
  _drawSpectrumAxisLabels(sampleRate, fftSize, minDb, maxDb, maxFreqHz) {
    ...
    for (let i = 0; i <= freqSteps; i++) {
      ...
      const label =
        freq >= 1e3 ? `${(freq / 1e3).toFixed(1)}k` : `${Math.round(freq)}`;
  ...
  _drawSpectrogramFreqLabels(maxFreqHz) {
    ...
    for (let i = 0; i <= steps; i++) {
      ...
      const label =
        freq >= 1e3 ? `${(freq / 1e3).toFixed(0)}k` : `${Math.round(freq)}`;
  ...
  drawHarmonicLadder(peaks, { minFreq = 20, maxFreq = 4e3 } = {}) {
    const { ctx, width, height, theme } = this;
    ctx.save();
    ctx.fillStyle = theme.background;
    ctx.fillRect(0, 0, width, height);
    ...
    const ys = peaks.map((p) =>
      this._logFreqToY(p.freq, minFreq, maxFreq, height),
    );
  ...
  drawHarmonicBalance(fundamentalLabel, oddRatio, thdPercent, reliable = true) {
    ...
    if (!reliable) {
      ...
      ctx.font = "12px ui-monospace, monospace";
      ctx.fillText(
        "Tone is near-pure - odd/even position not meaningful below ~2% THD",
        width / 2,
        trackY + 110,
      );
      ctx.restore();
  ...
  _composite() {
    ...
    if (theme.flicker) {
      ...
      flickerAlpha =
        0.94 +
        Math.sin(this._flickerPhase * 37) * 0.03 +
  ...
  drawCursors(cursors) {
    ...
    const hs = 4;
    if (cursors.vA != null)
      ctx.fillRect(cursors.vA - hs, 6 - hs, hs * 2, hs * 2);
    if (cursors.vB != null)
      ctx.fillRect(cursors.vB - hs, 6 - hs, hs * 2, hs * 2);
    if (cursors.hA != null)
      ctx.fillRect(6 - hs, cursors.hA - hs, hs * 2, hs * 2);
    if (cursors.hB != null)
      ctx.fillRect(6 - hs, cursors.hB - hs, hs * 2, hs * 2);
    ctx.restore();
  }
}

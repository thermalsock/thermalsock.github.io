...
let balanceSmooth = { oddRatio: 0.5, thdPercent: 0 };
...
els.startBtn.addEventListener("click", async () => {
  ...
  try {
    ...
    console.log(
      "[oscilloscope] Calling getUserMedia, deviceId =",
      deviceId || "(default)",
    );
...
function updateMeasurements(frameA) {
  ...
  els.measFreq.textContent = freqResult
    ? formatHz(freqResult.frequencyHz)
    : "--";
  els.measPeriod.textContent = freqResult
    ? formatMs(freqResult.periodMs)
    : "--";
}
...
function updateCursorReadout() {
  ...
  const msPerPixel =
    ((WINDOW_SIZE / audioEngine.sampleRate) * 1e3) / renderer.width;
  ...
  const deltaV = Math.abs(
    renderer.pixelToAmplitude(cursors.hA) -
      renderer.pixelToAmplitude(cursors.hB),
  );
...
function frame() {
  ...
  if (displayMode === "time") {
    ...
    els.measPeakFreq.textContent = "--";
    const freqResult = lastFrameA
      ? measureFrequency(lastFrameA, audioEngine.sampleRate)
      : null;
    updateHeroCard({
      label: "Frequency",
    ...
    updateHeroCard({
      ...
      unit: freqResult ? "Hz" : "",
      sub: showChannelB
        ? "Plotted against Ch B"
        : "Enable Ch B for a real Lissajous",
    });
    ...
    let maxIdx = 0;
    for (let i = 1; i < 12; i++)
      if (chromaSmooth[i] > chromaSmooth[maxIdx]) maxIdx = i;
    updateHeroCard({
    ...
    const freqBuffers = audioEngine.getFrequencyData();
    const rawPeaks = findSpectralPeaks(
      freqBuffers[0],
      audioEngine.sampleRate,
      audioEngine.fftSize,
      { minDb: audioEngine.minDecibels + 30, maxPeaks: 6 },
    );
    ...
    if (peaks.length > 0) {
      ...
      updateHeroCard({
        ...
        unit: "Hz",
        sub: note
          ? `${note.name}${note.octave} · ${note.cents >= 0 ? "+" : ""}${note.cents.toFixed(0)}¢ · ${peaks.length} peaks`
          : "",
      });
    } else {
      updateHeroCard({
    ...
    const rawPeaks = findSpectralPeaks(
      ...
      audioEngine.sampleRate,
      audioEngine.fftSize,
      { minDb: audioEngine.minDecibels + 30, maxPeaks: 1 },
    );
    if (rawPeaks.length > 0) {
      ...
      balanceSmooth.thdPercent =
        balanceSmooth.thdPercent * 0.8 + thdPercent * 0.2;
      ...
      renderer.drawHarmonicBalance(
        label,
        balanceSmooth.oddRatio,
        balanceSmooth.thdPercent,
        reliable,
      );
      updateHeroCard({
        ...
        unit: "%",
        sub: note
          ? `${note.name}${note.octave} fundamental · ${reliable ? (balanceSmooth.oddRatio > 0.5 ? "odd-dominant" : "even-dominant") : "near-pure tone"}`
          : "",
      });
    } else {

...
export function computeChroma(freqData, sampleRate, fftSize, {minFreq: minFreq = 80, maxFreq: maxFreq = 5e3} = {}) {
...
export function findSpectralPeaks(freqData, sampleRate, fftSize, {minDb: minDb = -70, maxPeaks: maxPeaks = 6, minSeparationHz: minSeparationHz = 15} = {}) {
  const binHz = sampleRate / fftSize;
  ...
  for (let i = 1; i < freqData.length - 1; i++) {
    ...
    if (db >= freqData[i - 1] && db >= freqData[i + 1]) {
      ...
      const p = denom !== 0 ? .5 * (alpha - gamma) / denom : 0;
      const interpBin = i + Math.max(-.5, Math.min(.5, p));
      candidates.push({
        bin: i,
        freq: interpBin * binHz,
        db: db
      });
    }
  }
...
export function computeHarmonicBalance(freqData, fundamentalHz, sampleRate, fftSize, {maxHarmonic: maxHarmonic = 16} = {}) {
  ...
  for (let h = 2; h <= maxHarmonic; h++) {
    ...
    if (h % 2 === 0) evenPower += power; else oddPower += power;
  }
  ...
  const oddRatio = reliable && totalHarmonicPower > 0 ? oddPower / totalHarmonicPower : .5;
  return {
    oddRatio: oddRatio,
...
export function describeRatio(freqLow, freqHigh) {
  ...
  const intervals = [ [ 3 / 2, "P5" ], [ 4 / 3, "P4" ], [ 5 / 4, "M3" ], [ 6 / 5, "m3" ], [ 5 / 3, "M6" ], [ 8 / 5, "m6" ], [ 9 / 8, "M2" ] ];
...
export function tagHarmonics(peaks, {tolerance: tolerance = .02} = {}) {

...
export function setContentCategory(category) {
  if (category !== "genres" && category !== "scales") return;
  if (contentState.contentCategory === category) return;
...
export function selectScaleType(index) {
  if (index < 0 || index >= SCALE_TYPES.length) return;
...
export function selectLesson(index) {
  const pack = getActivePack();
  if (index < 0 || index >= pack.lessons.length) return;

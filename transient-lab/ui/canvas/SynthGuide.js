...
function drawWaveIcon(ctx, cx, cy, r, waveType) {
  ...
  if (waveType === "sine") {
    for (let i = 0; i <= 16; i++) {
      const t = i / 16;
      const x = left + t * w;
      const y = cy - Math.sin(t * Math.PI * 2) * (r * .5);
      i === 0 ? ctx.moveTo(x, y) : ctx.lineTo(x, y);
    }
  } else if (waveType === "saw") {
    ctx.moveTo(left, cy + r * .5);
    ctx.lineTo(left, cy - r * .5);
    ctx.lineTo(cx, cy + r * .5);
    ctx.lineTo(cx, cy - r * .5);
    ctx.lineTo(left + w, cy + r * .5);
...
function drawKnob(ctx, cx, cy, r, value01) {
  ...
  ctx.beginPath();
  ctx.moveTo(cx, cy);
  ctx.lineTo(cx + Math.cos(valueAngle) * r * .55, cy + Math.sin(valueAngle) * r * .55);
  ctx.strokeStyle = theme.accentDeep;
  ctx.lineWidth = Math.max(1.3, r * .09);
  ctx.stroke();
  ctx.lineWidth = 1;
}
...
export function drawSynthGuide(ctx, canvas) {
  ...
  const bigR = smallR * 1.85;
  {
    ...
    [ guide.osc1, guide.osc2 ].forEach((osc, i) => {
      const cx = content.x + content.w * (.28 + i * .44);
      ...
      ctx.font = "9px -apple-system, system-ui, sans-serif";
      ctx.fillStyle = theme.textSecondary;
      wrapText(ctx, osc.label, cx, iconY + smallR + 14, content.w * .42, 10, 2);
      ctx.textAlign = "left";
    });
  }
  {
    const content = drawSectionBox(ctx, boxes[1].x, boxes[1].y, boxes[1].w, boxes[1].h, "MIXER");
    const knobR = smallR * .85;
  ...
  }
  {
    ...
    const smallKnobR = smallR * .78;
    const rowY = content.y + content.h / 2;
    const cx = content.x + content.w / 2;
    labeledKnob(ctx, cx - content.w * .28, rowY, smallKnobR, guide.filter.drive, "Drive");
    labeledKnob(ctx, cx, rowY, bigR, guide.filter.cutoff, "Cutoff");
    labeledKnob(ctx, cx + content.w * .28, rowY, smallKnobR, guide.filter.resonance, "Res");
  }
  {
    ...
    [ {
      ...
      tag: "FLT",
      env: guide.filterEnv,
      cy: firstRowCenterY + rowH + gapBetweenRows
    } ].forEach(({tag: tag, env: env, cy: cy}) => {
      ctx.font = "bold 9px -apple-system, system-ui, sans-serif";
      ctx.fillStyle = theme.textDim;
      ctx.textAlign = "left";

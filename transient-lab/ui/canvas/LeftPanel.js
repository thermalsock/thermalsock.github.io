// LeftPanel.js
//
// Ported structure from Rudiment: every control's label is positioned
// relative to its OWN box (from getLeftPanelLayout()), never a
// separately hand-incremented y — that discipline is what prevented
// Rudiment's label-drift bug and it carries over here unchanged.
//
// Restructured: Game Mode/Sandbox removed (nothing here scores
// anything, so it never actually applied) and replaced with a
// Scales/Genres category toggle -- the real top-level choice this app
// has. A new Pack selector sits below it, its own dropdown, separate
// from Now Learning's lesson list -- picking WHICH pack is now its own
// step rather than nested tabs inside the lesson dropdown.
...
export function drawLeftPanel(ctx, canvas) {
  ...
  if (!connected) {
    ctx.fillText(
      midiState.supported ? "No synth connected" : "MIDI unavailable",
      midiBox.x + 7, midiBox.y + 19
    );
  } else {
    const line1 = deviceNames[0];
    let line2 = null;
    if (deviceNames.length === 2) line2 = deviceNames[1];
    else if (deviceNames.length > 2) line2 = `+${deviceNames.length - 1} more`;
  ...
  // ---- CATEGORY SELECTOR — Scales / Genres ----
  ...
  // ---- SCALE TYPE SELECTOR (only exists when Scales is active) ----
  // "Pick what kind of scale, then pick the scale" -- a genuine
  // two-step hierarchy (Major/Minor/Modes/Pentatonic/Blues, then the
  // specific scale within that type), matching how real scale
  // reference material is actually organized.
  if (inScales && layout.scaleTypeBox) {
    ...
    if (contentState.scaleTypeDropdownOpen) {
      ...
      ctx.font = "12px -apple-system, system-ui, sans-serif";
      return; // everything below stays hidden while this list is open
    }
  }

  // ---- PACK SELECTOR (its own dropdown, separate from Now Learning) ----
  ...
  // Small caret to hint this opens a list, same affordance language as
  // the "(change ▾)" text already used for the audio device picker.
  ctx.font = "10px -apple-system, system-ui, sans-serif";
  ctx.fillStyle = theme.textDim;
  ctx.textAlign = "right";
  ctx.fillText("▾", packBox.x + packBox.w - 8, packBox.y + 22);
  ctx.textAlign = "left";
  ctx.font = "12px -apple-system, system-ui, sans-serif"; // reset -- BPM's label below was silently inheriting this 10px caret font instead of the intended 12px, which is why it looked like a different font entirely
  ...
  if (contentState.packDropdownOpen) {
    ...
    ctx.fillRect(0, dropdownBottom, leftPanelWidth, canvas.height - dropdownBottom);
    ctx.font = "12px -apple-system, system-ui, sans-serif";
    return; // everything below (BPM, Now Learning, transport, watermark) stays hidden while this list is open
  }
  ...
  // ---- NOW LEARNING — just the lesson list now, no nested pack tabs ----
  ...
  // Same pattern as Rudiment's pattern dropdown: while open, the list
  // takes over the rest of the panel (paving over transport/watermark
  // with panelBg) rather than trying to squeeze both in.
...
function drawTransport(ctx, layout) {
  ...
  transportButton(t.reset, (cx, cy) => {
    ctx.lineWidth = 1.8;
    ctx.beginPath();
    ctx.arc(cx, cy, 6, -Math.PI * 0.15, Math.PI * 1.35);
    ctx.stroke();
...
// Returns "toggle" if the scale-type box itself was clicked, a scale-
// type index (number) if a row inside the open list was clicked, or
// null. Returns null unconditionally when Scales isn't the active
// category, since the box doesn't exist in that state at all.
export function hitTestScaleTypeSelector(x, y) {
  ...
  if (contentState.scaleTypeDropdownOpen) {
    ...
    for (let i = 0; i < SCALE_TYPES.length; i++) {
      const itemBox = { x: stBox.x, y: listStartY + i * 26, w: stBox.w, h: 24 };
...
// Returns "toggle" if the pack box itself was clicked, or a pack index
// (number) if a row inside the open pack list was clicked, or null.
export function hitTestPackSelector(x, y) {
  ...
  if (contentState.packDropdownOpen) {
    ...
    for (let i = 0; i < packs.length; i++) {
      const itemBox = { x: packBox.x, y: listStartY + i * 26, w: packBox.w, h: 24 };
      if (pointInBox(x, y, itemBox)) return i;
    }
  }
  return null;
...
// Returns "toggle" if the Now Learning box itself was clicked, or a
// lesson index (number) if a row inside the open dropdown was clicked,
// or null for neither.
export function hitTestNowLearning(x, y) {
  ...
  if (contentState.lessonDropdownOpen) {
    ...
    for (let i = 0; i < pack.lessons.length; i++) {
      const itemBox = { x: nlBox.x, y: startY + i * 26, w: nlBox.w, h: 24 };
      if (pointInBox(x, y, itemBox)) return i;
    }
  }

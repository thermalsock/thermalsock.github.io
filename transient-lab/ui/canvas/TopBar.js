...
function findCurrentAndNext(timeline, currentBeat) {
  ...
  const next = events.length ? events[(currentIndex + 1) % events.length] : null;
  return {
    current: current,
    next: next
  };
...
export function drawTopBar(ctx, canvas, timeline, currentBeat) {
  ...
  const {current: current, next: next} = findCurrentAndNext(timeline, currentBeat);
  let cursorX = x + 20;

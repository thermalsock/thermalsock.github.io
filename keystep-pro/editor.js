  ...
  function create(opts) {
    ...
    function refreshMeta(p) {
      ...
      p.steps.forEach(function(s) {
        if (!s.rest && s.notes) s.notes.forEach(function(n) {
          used.push(n);
        });
      });
      p.meta.onsetCount = p.steps.filter(function(s) {
        return !s.rest;
      }).length;
      if (used.length) {
    ...
    function nudgePitch(degrees, chromatic) {
      ...
      if (!chromatic && s.degree !== undefined && m.scalePcs && m.rootMidi !== undefined) {
        ...
        s.notes = s.notes.map(function(n) {
          return clampNote(n + shift);
        });
      } else {
        s.notes = s.notes.map(function(n) {
          return clampNote(n + degrees);
        });
      }
      changed();
    }
    ...
    function setGate(g) {
      ...
      if (!s || s.rest) return;
      snapshot();
      s.gate = Math.max(.05, Math.min(1.9, g));
      changed();
    }
    ...
    function setShift(v) {
      ...
      if (!s || s.rest) return;
      snapshot();
      s.shift = Math.max(-.49, Math.min(.49, v));
      changed();
    }
    function toggleTie() {
      var s = step(sel);
    ...
    function lockedIndices() {
      return Object.keys(locks).map(Number).sort(function(a, b) {
        return a - b;
      });
    }
    function lockAll() {

/* Thermalsock Labs — KeyStep Pro Arp Machine, app layer.
 *
 * Holds the generated pattern, paints it onto the panel and the step grid,
 * and plays it back. The generator does not know this file exists; this file
 * does not decide anything musical. Keeping that line clean is what will let
 * the injector drop in later without touching either.
 */
(function () {
  'use strict';
  ...
  var panel = null;
  var current = null;      /* the confirmed 16-step seed */
  var developed = null;    /* the 64-step expansion, once built */
  var showing = null;      /* whichever is on screen */
  ...
  var STYLE_GROUPS = [
    { label: 'Sequencer classics', keys: ['berlin', 'motorik', 'acid', 'electro', 'detroit', 'trance'] },
    { label: 'Ambient and modal',  keys: ['neoambient', 'psybient', 'cinematic', 'pulse'] },
    { label: 'Minimalism',         keys: ['phase', 'additive'] },
    { label: 'Broken and swung',   keys: ['idm', 'breakbeat', 'dubtechno'] },
    { label: 'Retro',              keys: ['synthwave', 'arpeggio'] }
  ];
  ...
  function initControls() {
    STYLE_GROUPS.forEach(function (grp) {
      ...
      grp.keys.forEach(function (k) {
        ...
        var o = document.createElement('option');
        o.value = k; o.textContent = G.STYLES[k].name;
        og.appendChild(o);
      });
    ...
    G.NOTE_NAMES.forEach(function (n, i) {
      var o = document.createElement('option');
      o.value = i; o.textContent = n; rt.appendChild(o);
    });

    /* Restore the last session — people iterate on one idea across sittings. */
    var saved = store && store.get('controls', null);
    if (saved) {
      ['style', 'role', 'scale', 'root', 'octave', 'density', 'range', 'tempo',
     'mode', 'track', 'clockMode', 'period', 'phrasing', 'lengthStrategy', 'phraseGrid',
     'ensCount', 'ensPreset', 'ensSpace'].forEach(function (k) {
  ...
  /* Picking a style suggests the scale it grew up in, but never forces it —
     a Berlin line in Mixolydian is a legitimate thing to want. */
  ...
  function updateGenerateLabel() {
    ...
    /* Two strategies pick their length at random, so promising an exact
       number on the button would be a small lie every other press. */
    ...
    /* The length strategy only means something once there is a second phrase
       to have a length, so it disappears for through-composed. */
  ...
  function renderPhraseMap(pattern) {
    ...
    ph.forEach(function (f) {
      ...
      /* Showing material against length is the whole point: it is the
         difference between a short answer and a truncated one. */
      t.textContent = (f.material && f.material < f.length)
        ? f.material + ' of ' + f.length + ' steps'
        : f.length + ' steps';
  ...
  function paint(pattern) {
    ...
    /* Panel: steps light for the first 16, keys light for the scale. */
    panel.clearSteps();
    for (var i = 0; i < 16; i++) {
    ...
    if (editor) { decorateGrid(); renderStepEditor(); }
  ...
  function renderGrid(pattern) {
    ...
    pattern.steps.forEach(function (s, i) {
      ...
      bars.appendChild(v); bars.appendChild(g);
      ...
      cell.addEventListener('mousedown', function (ev) {
        ...
        editor.select(i);
        if (s.rest) return;
        /* Drag up and down to change pitch. Faster than the buttons once you
           know the shape you are after, and it keeps your eye on the grid. */
      ...
      if (!s.rest) {
        cell.title = G.noteName(s.notes[0]) +
          (s.notes.length > 1 ? ' + ' + s.notes.slice(1).map(G.noteName).join(' ') : '') +
          '  ·  velocity ' + s.vel + '  ·  gate ' + s.gate.toFixed(2) + (s.tie ? '  ·  tied' : '');
      }
  ...
  function renderMeta(pattern) {
    var m = pattern.meta;
    /* Polyrhythm voices come from a different engine and have no contour,
       phrasing or style seed to report. Listing the fields that exist beats
       printing "contour: undefined" next to a perfectly good pattern. */
    var bits = [
      m.styleName, m.roleName, m.rootName + ' ' + m.scaleName,
      m.onsetCount + ' of ' + m.length + ' steps',
      m.lowName + '–' + m.highName
    ];
    if (m.phrasingName && m.phrases && m.phrases.length > 1) {
      bits.push(m.phrasingName + ', ' + m.phrases.map(function (f) { return f.length; }).join('+'));
    }
    ...
    /* If the scale is not one the KeyStep Pro can quantise to, say so — the
       hardware will silently bend these notes if its scale engine is on. */
    var warn = $('scaleWarn');
    if (!m.onDevice) {
      warn.style.display = '';
      warn.textContent = m.scaleName + ' is not in the KeyStep Pro\u2019s own scale list. ' +
        'Set the hardware scale to Chromatic before injecting, or the box will quantise these notes to something else.';
    } else warn.style.display = 'none';

    var ops = $('ops');
    if (m.operations) {
      ops.style.display = '';
      ops.innerHTML = '<b>How the 64 was built from your 16</b>' +
        m.operations.map(function (o) {
  ...
  /* ------------------------------------------------------------------
     Audition. A deliberately plain voice — the point is to judge the notes,
     not to be impressed by the synth.
     ------------------------------------------------------------------ */

  var ac = null, playTimer = null, playPos = 0;

  function voiceFor(role) {
    if (role === 'bass') return { wave: 'sawtooth', cutoff: 900, q: 6, decay: 0.9, gain: 0.30 };
    if (role === 'pad') return { wave: 'triangle', cutoff: 2200, q: 1, decay: 1.6, gain: 0.13 };
    if (role === 'counter') return { wave: 'square', cutoff: 1800, q: 3, decay: 0.7, gain: 0.16 };
    return { wave: 'sawtooth', cutoff: 2600, q: 4, decay: 0.8, gain: 0.18 };
  }
  ...
  function playNote(midi, vel, dur, when, v) {
    ...
    filt.type = 'lowpass';
    filt.frequency.setValueAtTime(v.cutoff * (0.5 + vel / 127), when);
    filt.frequency.exponentialRampToValueAtTime(Math.max(180, v.cutoff * 0.35), when + dur * 0.9);
    filt.Q.value = v.q;
    var peak = v.gain * (0.35 + vel / 127 * 0.65);
    amp.gain.setValueAtTime(0.0001, when);
    amp.gain.exponentialRampToValueAtTime(peak, when + 0.006);
    amp.gain.exponentialRampToValueAtTime(0.0001, when + Math.max(0.05, dur));
    osc.connect(filt); filt.connect(amp); amp.connect(ac.destination);
    osc.start(when); osc.stop(when + Math.max(0.06, dur) + 0.02);
  }
  ...
  function play() {
    ...
    var stepSec = 60 / bpm / 4;                 /* 1/16 notes */
    var v = voiceFor(showing.meta.role);
    playPos = 0;
    $('play').textContent = 'Stop';
    $('play').setAttribute('data-playing', 'yes');

    var startAt = ac.currentTime + 0.08;
    var scheduled = 0;

    /* Schedule ahead in chunks rather than one note per timer tick, so the
       playback does not stutter when the tab is busy. */
    function pump() {
      var horizon = ac.currentTime + 0.35;
      while (scheduled < showing.steps.length && startAt + scheduled * stepSec < horizon) {
        ...
        if (!s.rest) {
          ...
          for (var r = 0; r < reps; r++) {
            (function (when) {
              s.notes.forEach(function (n) { playNote(n, s.vel, dur, when, v); });
            })(base + r * (stepSec / reps));
          }
        }
        scheduled++;
      ...
      if (scheduled >= showing.steps.length && elapsed > showing.steps.length * stepSec) {
        if ($('loop').checked) { stop(true); play(); return; }
        stop(); return;
      }
  ...
  function generate(newSeed) {
    ...
    $('develop').textContent = reps > 1
      ? 'Confirm \u2014 develop to ' + devLen + ' (' + reps + ' statements)'
      : 'Confirm \u2014 develop this';
  ...
  function initEditor() {
    /* Editing and the library are enhancements. If either script is missing
       the generator still has to work — losing a feature is a nuisance, a
       blank page is a bug report. */
    if (!window.KSPEdit || !$('stepEditor')) return;
    editor = window.KSPEdit.create({
      getPattern: function () { return showing; },
      onChange: function (kind) {
        if (kind === 'select' || kind === 'lock') { decorateGrid(); renderStepEditor(); }
        else { paint(showing); renderStepEditor(); }
    ...
    $('lockAll').addEventListener('click', function () { editor.lockAll(); });
    $('lockClear').addEventListener('click', function () { editor.clearLocks(); });
    $('undo').addEventListener('click', function () { editor.undo(); });
    $('redo').addEventListener('click', function () { editor.redo(); });
    $('seClose').addEventListener('click', function () { editor.select(null); });
    ...
    Array.prototype.forEach.call(document.querySelectorAll('[data-pitch]'), function (b) {
      b.addEventListener('click', function () {
        ...
        if (Math.abs(v) === 12) editor.nudgeOctave(v > 0 ? 1 : -1);
        else editor.nudgePitch(v);
      });
    });
    $('seVel').addEventListener('input', function () { editor.setVelocity(+$('seVel').value); });
    $('seGate').addEventListener('input', function () { editor.setGate(+$('seGate').value / 100); });
    $('seShift').addEventListener('input', function () { editor.setShift(+$('seShift').value / 100); });
    $('seRatchet').addEventListener('input', function () { editor.setRatchet(+$('seRatchet').value); });
    $('seRest').addEventListener('click', function () { editor.toggleRest(); });
    $('seLock').addEventListener('click', function () { editor.toggleLock(); });
    $('seTie').addEventListener('click', function () { editor.toggleTie(); });
    $('seAddNote').addEventListener('click', function () { editor.addNote(); });
    $('seDelNote').addEventListener('click', function () { editor.removeNote(); });

    /* Keyboard. Held back whenever focus is in a field, or typing a seed
       would transpose the pattern. */
    document.addEventListener('keydown', function (ev) {
      ...
      switch (ev.key) {
        case 'ArrowUp':    ev.preventDefault(); ev.shiftKey ? editor.nudgeOctave(1) : editor.nudgePitch(1, ev.altKey); break;
        case 'ArrowDown':  ev.preventDefault(); ev.shiftKey ? editor.nudgeOctave(-1) : editor.nudgePitch(-1, ev.altKey); break;
        case 'ArrowLeft':  ev.preventDefault(); editor.move(-1); break;
        case 'ArrowRight': ev.preventDefault(); editor.move(1); break;
        case 'Escape':     editor.select(null); break;
        case 'r': case 'R': editor.toggleRest(); break;
        case 'l': case 'L': editor.toggleLock(); break;
        default: break;
      }
    });
  }

  /* Reroll everything except what has been locked. */
  function rerollUnlocked() {
    if (!editor || !current) { generate(true); return; }
    stop();
  ...
  function renderStepEditor() {
    ...
    if (i < 0 || !showing || !showing.steps[i]) { box.hidden = true; return; }
    box.hidden = false;
    ...
    $('seGate').value = Math.round((s.gate || 0.5) * 100);
    $('seGateOut').textContent = s.rest ? '\u2014' : (s.gate || 0.5).toFixed(2);
    $('seShift').value = Math.round((s.shift || 0) * 100);
    $('seShiftOut').textContent = s.rest ? '\u2014'
      : ((s.shift || 0) === 0 ? 'on the grid' : ((s.shift > 0 ? '+' : '') + Math.round(s.shift * 100) + '%'));
  ...
  function renderLibrary() {
    ...
    var u = LIB.usage();
    $('libUsage').textContent = u.entries
      ? u.entries + ' saved \u00b7 ' + Math.round(u.bytes / 1024) + ' KB'
      : '';
    ...
    items.forEach(function (e) {
      ...
      sub.textContent = (e.kind === 'ensemble'
        ? e.tracks.length + ' tracks \u00b7 ' + e.lengths.join('/') + ' \u00b7 cycle ' + e.cycle
        : e.meta.onsetCount + ' notes \u00b7 ' + e.meta.lowName + '\u2013' + e.meta.highName)
      ...
      /* Regenerating from the recipe is how you get more like this one —
         a different seed, everything else held. */
      if (e.recipe) {
        act('More like this', function () {
          ...
          syncStyleHint(); updateReadouts(); updateGenerateLabel();
          generate(false);
          $('panelLib').open = false;
        });
      }
      ...
      act('Rename', function () {
        ...
        if (next && next.trim()) { LIB.rename(e.id, next.trim()); renderLibrary(); }
      });

      act('Delete', function () {
        if (confirm('Delete \u201c' + e.name + '\u201d? This cannot be undone.')) {
          LIB.remove(e.id); renderLibrary();
        }
      }, 'del');
  ...
  function saveCurrent() {
    ...
    if (!entry) { alert('Could not save — browser storage is full. Export and clear some space.'); return; }
    renderLibrary();
    flash($('save'), 'Saved');
  }
  ...
  function saveEnsembleNow() {
    if (!ens || !LIB) return;
    var name = prompt('Name this ensemble',
      'Polyrhythm \u00b7 ' + ens.lengths.join('/') + ' \u00b7 ' + ens.tracks.length + ' tracks');
    if (name === null) return;
    var entry = LIB.saveEnsemble(ens, readControlValues(), name.trim());
    if (!entry) { alert('Could not save — browser storage is full.'); return; }
    renderLibrary();
    flash($('ensSave'), 'Saved');
  }

  /* The recipe is what the controls said, so "more like this" can restore it. */
  function readControlValues() {
    var out = {};
    ['style', 'role', 'scale', 'root', 'octave', 'density', 'range', 'seed',
     'phrasing', 'lengthStrategy', 'phraseGrid', 'tempo',
     'ensCount', 'ensPreset', 'ensSpace', 'ensSeed'].forEach(function (k) {
      if ($(k)) out[k] = $(k).value;
    });
    return out;
  }
  ...
  function ensGenerate() {
    ...
    ens = G.ensemble({
      ...
      style: c.style, scale: c.scale, root: c.root,
      octave: c.octave, range: c.range, density: c.density
    });
    renderEnsemble();
    saveControls();
  }
  ...
  function renderEnsemble() {
    ...
    $('ensCycle').textContent =
      'lengths ' + ens.lengths.join(' / ') +
      '   \u00b7   realigns after ' + ens.cycle + ' steps (' +
      (bars % 1 === 0 ? bars : bars.toFixed(1)) + ' bars of 16)' +
      '   \u00b7   at most ' + ens.maxStack + ' part' + (ens.maxStack === 1 ? '' : 's') + ' on any step';
    ...
    ens.tracks.forEach(function (t) {
      ...
      info.textContent = t.pattern.meta.roleName + '  \u00b7  ' + t.length + ' steps  \u00b7  ' +
        t.pattern.meta.onsetCount + ' notes  \u00b7  ' +
        t.pattern.meta.lowName + '\u2013' + t.pattern.meta.highName;
      hd.appendChild(info);
  ...
  /* Auditioning the ensemble is the only way to hear whether the drift works.
     Each track loops at its own length, so they come apart and back together
     exactly as they will on the hardware. */
  var ensTimer = null, ensStart = 0;

  function ensPlay() {
    if (!ens) return;
    if (ensTimer) { stopEns(); return; }
    if (!ac) ac = new (window.AudioContext || window.webkitAudioContext)();
    if (ac.state === 'suspended') ac.resume();
    stop();
    ...
    var voices = ens.tracks.map(function (t) { return voiceFor(t.role); });
    var scheduled = ens.tracks.map(function () { return 0; });
    ensStart = ac.currentTime + 0.1;
    $('ensPlay').textContent = 'Stop';

    function pump() {
      var horizon = ac.currentTime + 0.4;
      ens.tracks.forEach(function (t, ti) {
        while (ensStart + scheduled[ti] * stepSec < horizon) {
          ...
          if (s && !s.rest) {
            ...
            for (var r = 0; r < reps; r++) {
              (function (w) {
                s.notes.forEach(function (n) { playNote(n, s.vel, d2, w, voices[ti]); });
              })(when + r * (stepSec / reps));
            }
          }
          scheduled[ti]++;
  ...
  function stopEns() {
    if (ensTimer) { clearTimeout(ensTimer); ensTimer = null; }
  ...
  /* A button labelled Send has to send. The earlier version loaded the part
     and opened the Send panel, which from the outside looked like the section
     collapsing and nothing happening.

     It still refuses to fire blind: preflight runs first, and if anything is
     wrong — no device, wrong port — it opens the panel with the reason rather
     than throwing notes at a port that is not listening. Progress is reported
     next to the button that was pressed, because the Send panel it would
     normally write to is collapsed at that moment. */
  var statusSink = null;
  ...
  function sendFrom(pattern, trackIndex, what, sink) {
    ...
    var blocked = window.KSPInject.preflight(showing, {
      output: currentOutput(), mode: mode()
    }).some(function (it) { return it.level === 'stop'; });

    if (blocked) {
      $('panelSend').open = true;
      refreshPreflight();
      if ($('panelSend').scrollIntoView) $('panelSend').scrollIntoView({ behavior: 'smooth', block: 'start' });
  ...
  function initPoly() {
    ...
    Object.keys(P.PRESETS).forEach(function (k) {
      var o = document.createElement('option');
      o.value = k; o.textContent = P.PRESETS[k].name;
      $('prPreset').appendChild(o);
    });
    var custom = document.createElement('option');
    custom.value = 'custom'; custom.textContent = 'Custom \u2014 set below';
    $('prPreset').appendChild(custom);
    $('prPreset').value = '3:4';

    Object.keys(P.PITCH_MODES).forEach(function (k) {
      var o = document.createElement('option');
      o.value = k; o.textContent = P.PITCH_MODES[k].name;
    ...
    $('prPreset').addEventListener('change', function () {
      ...
      if (pre) {
        prSpecs = pre.pulses.map(function (n) { return { pulses: n }; });
        renderVoiceControls();
      }
    });
    $('prMethod').addEventListener('change', function () { renderVoiceControls(); prBuild(); });
  ...
  function renderVoiceControls() {
    ...
    prSpecs.forEach(function (spec, i) {
      ...
      if (division) {
        ...
        Object.keys(P.DIVISIONS).forEach(function (k) {
          var o = document.createElement('option');
          o.value = k; o.textContent = P.DIVISIONS[k].name;
          sel.appendChild(o);
        });
        sel.value = spec.division || ['e', 'et', 's', 'q'][i % 4];
        spec.division = sel.value;
        sel.style.width = '110px';
        sel.addEventListener('change', function () { spec.division = sel.value; prBuild(); });
        box.appendChild(sel);
      } else {
        var pulses = document.createElement('input');
        pulses.type = 'number'; pulses.min = 1; pulses.max = 32;
        ...
        rot.type = 'number'; rot.min = 0; rot.max = 63;
      ...
      if (prSpecs.length > 2) {
        ...
        kill.addEventListener('click', function () {
          prSpecs.splice(i, 1); renderVoiceControls(); prBuild();
        });
    ...
    if (prSpecs.length < 4) {
      ...
      add.addEventListener('click', function () {
        prSpecs.push({ pulses: [5, 7, 9][prSpecs.length - 2] || 5 });
        $('prPreset').value = 'custom';
        renderVoiceControls(); prBuild();
      });
  ...
  function prBuild() {
    ...
    poly = P.build({
      ...
      root: c.root, scale: c.scale, octave: c.octave,
      seed: c.seed,
      voices: prSpecs.map(function (sp) {
        return Object.assign({}, sp, { pitchMode: $('prPitch').value });
      })
    });
    renderPoly();
  }
  ...
  function renderPoly() {
    ...
    var meet = {};
    poly.coincide.forEach(function (c) { meet[c.at] = c.voices; });

    $('prAnalysis').textContent =
      poly.note + '  The voices realign every ' + poly.period + ' pulses' +
      (poly.method === 'division' ? '' : ', which is ' + (poly.period / poly.counts[0]).toFixed(2) +
        ' cycles of voice 1') + '.' +
      (poly.coincide.length
        ? '  They touch at step' + (poly.coincide.length === 1 ? ' ' : 's ') +
          poly.coincide.map(function (c) { return c.at + 1; }).join(', ') + '.'
        : '  Nothing coincides after the downbeat.');
    ...
    poly.voices.forEach(function (v) {
      ...
      hd.appendChild(b);
      var info = document.createElement('span');
      info.textContent = (v.division ? P.DIVISIONS[v.division].name + '  \u00b7  ' : '') +
        v.pattern.meta.onsetCount + ' hits over ' + v.length + ' steps  \u00b7  ' +
        P.PITCH_MODES[v.pitchMode].name;
      hd.appendChild(info);
    ...
    $('prPlan').innerHTML = '<ol>' + plan.map(function (t) {
      return '<li>Track ' + t.track + ': set <code>' + t.division + '</code> and <code>Lst Step ' +
             t.lastStep + '</code>. ' + t.note + '</li>';
    }).join('') + '</ol>';
  }
  ...
  function prPlay() {
    ...
    var scheduled = poly.voices.map(function () { return 0; });
    var start = ac.currentTime + 0.1;
  ...
  function prSave() {
    ...
    var asEns = {
      tracks: poly.voices.map(function (v) {
        return { index: v.index, role: 'voice', length: v.length, pattern: v.pattern };
      }),
      lengths: poly.voices.map(function (v) { return v.length; }),
      cycle: poly.period
    };
    if (!LIB.saveEnsemble(asEns, { prRatio: poly.ratio, prMethod: poly.method })) {
  ...
  function currentInput() {
    /* The site keeps one device choice in TSDevices, set from the home page
       System panel. Honour it before falling back to this app's own guess, so
       arriving here from another tool does not mean choosing again. */
  ...
  /* The device-side setup differs per method, and getting it wrong is the
     main way an injection fails. Spell it out rather than assume. */
  function renderSetupSteps() {
    ...
    if (mode() === 'realtime') {
      lines = [
        'Select <code>Track ' + track + '</code> and set its input MIDI channel to <code>' + track + '</code>.',
        'Set <code>Lst Step</code> to <code>' + len + '</code>.',
        'Set time division to <code>1/16</code> and turn quantise on.'
      ];
      if ($('clockMode').value === 'master') {
        lines.push('Set the KeyStep Pro sync source to <code>USB</code> so it follows this tab\u2019s clock.');
        lines.push('Press <code>Record</code> to arm it \u2014 no need to press Play. ' +
                   'This tab sends the transport: a four-beat count-in of clock, then Start, ' +
                   'then the notes, so the device begins writing at step 1.');
      } else {
        ...
        lines.push('Press Send to arm, <em>then</em> hold <code>Record</code> and press <code>Play</code>. ' +
                   'The notes are placed on the device\u2019s own clock pulses, so tempo here does not matter.');
      }
    } else {
      lines = [
        'Select <code>Track ' + track + '</code> and set its input MIDI channel to <code>' + track + '</code>.',
        'Set <code>Lst Step</code> to <code>' + len + '</code>.',
        'Clear the pattern with <code>Shift</code> + step 1.',
        'Press <code>Record</code> so Step Record arms and the display sits on step 1. Do not press Play.',
        'Press Send. It takes about ' +
          (Math.round(len * parseInt($('period').value, 10) / 100) / 10) + ' seconds.'
      ];
    }
    host.innerHTML = '<b>On the hardware, before you press Send</b><ol>' +
  ...
  function doSend() {
    ...
    if (!outPort) { setSendStatus('No MIDI output.'); return; }
    ...
    if (mode() === 'realtime' && $('clockMode').value === 'follow') {
      ...
      if (!inPort) {
        setSendStatus(
          'Follow mode needs the KeyStep Pro MIDI input as well as its output. ' +
          'Switch to "Drive the KeyStep Pro", or check the device is connected both ways.');
        return;
      }
      ...
      injector.followRecord(showing, inPort, {
        onStart: function () { setSendStatus('Recording\u2026'); },
        onStep: function (i, total) {
          setSendStatus('Recording\u2026 step ' + (i + 1) + ' of ' + total);
          if (i < 16) panel.setStep(i + 1, 'cursor');
        },
        onStop: function (n) { finishSend({ stepsWritten: n }); }
      });
      return;
    }
    ...
    function tick() {
      ...
      if (mode() === 'realtime' && elapsed < plan.countInMs) {
        ...
        setSendStatus('Sending\u2026 ' + (left / 1000).toFixed(1) + 's left');
      }
      if (left <= 0) { finishSend(plan); return; }
  ...
  function finishSend(plan) {
    ...
    if (plan && plan.restsDropped) {
      msg += plan.stepsWritten + ' steps written, ' + plan.restsDropped +
             ' rests dropped \u2014 the line is compacted.';
    } else {
  ...
  function abortSend() {
    if (injector) { injector.disarm(); injector.panic(); }
    if (sendTimer) { clearTimeout(sendTimer); sendTimer = null; }
    $('send').style.display = '';
    $('abort').style.display = 'none';
  ...
  function initMidi() {
    if (!navigator.requestMIDIAccess) { refreshPreflight(); return; }
    function grab() {
    ...
    if (navigator.permissions && navigator.permissions.query) {
      navigator.permissions.query({ name: 'midi', sysex: false })
        .then(function (st) { if (st.state === 'granted') grab(); else refreshPreflight(); })
        .catch(grab);
    } else grab();
  }
  ...
  function init() {
    ...
    $('generate').addEventListener('click', function () { generate(true); });
    $('reroll').addEventListener('click', function () { generate(false); });
    $('develop').addEventListener('click', develop);
    $('back').addEventListener('click', function () { stop(); paint(current); $('back').style.display = 'none'; });
    ...
    Object.keys(G.LENGTH_PRESETS).forEach(function (k) {
      var o = document.createElement('option');
      o.value = k; o.textContent = G.LENGTH_PRESETS[k].name;
    ...
    /* Accordion. Opening one panel closes the others, so the page stays a
       single column of one thing at a time rather than a wall of controls.
       Auditioning stops when its panel is folded away — hearing a sequence you
       can no longer see is disorienting. */
    var panels = [$('panelLib'), $('panelPoly'), $('panelPolyR'), $('panelSend')];
    panels.forEach(function (p) {
      if (!p) return;
      p.addEventListener('toggle', function () {
        if (p.open) {
          panels.forEach(function (o) { if (o !== p && o.open) o.open = false; });
    ...
    panel.on('step', function (d) {
      ...
      $('meta').textContent = s && !s.rest
        ? 'Step ' + d.step + ': ' + s.notes.map(G.noteName).join(' + ') +
          '   ·   velocity ' + s.vel + '   ·   gate ' + s.gate.toFixed(2)
        : 'Step ' + d.step + ': rest';
    });

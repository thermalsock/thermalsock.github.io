(function(global) {
  "use strict";
  var WORKLET_SOURCE = [ "class TapProcessor extends AudioWorkletProcessor {", "  process(inputs) {", "    const input = inputs[0];", "    if (input && input.length) {", "      // Copy: the render quantum buffers are reused between calls.", "      const chans = [];", "      for (let c = 0; c < input.length; c++) chans.push(new Float32Array(input[c]));", "      this.port.postMessage(chans);", "    }", "    return true;", "  }", "}", 'registerProcessor("ts-tap", TapProcessor);' ].join("\n");
  ...
  Recorder.prototype.start = function() {
    ...
    function useScriptProcessor() {
      ...
      node.onaudioprocess = function(e) {
        self._collect([ new Float32Array(e.inputBuffer.getChannelData(0)), new Float32Array(e.inputBuffer.getChannelData(e.inputBuffer.numberOfChannels > 1 ? 1 : 0)) ]);
      };
    ...
    if (self.ctx.audioWorklet) {
      var url = URL.createObjectURL(new Blob([ WORKLET_SOURCE ], {
        type: "application/javascript"
      }));
      return self.ctx.audioWorklet.addModule(url).then(function() {
        URL.revokeObjectURL(url);
        var node = new AudioWorkletNode(self.ctx, "ts-tap", {
          numberOfOutputs: 0
        });
        node.port.onmessage = function(e) {
          self._collect(e.data);
        };
        self.source.connect(node);
        self.node = node;
      }).catch(function() {
  ...
  global.TSRecorder = {
    create: function(ctx, sourceNode) {
      return new Recorder(ctx, sourceNode);
    }
  };
})(window);